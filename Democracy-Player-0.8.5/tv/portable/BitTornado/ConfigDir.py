...
# sys.argv isn't available on Democracy Windows-XUL, and there's no chance
# we're using the old icon path, so I'm commenting this out. --NN
...
class ConfigDir:
    ...
    def __init__(self, config_type = None):
        ...
        dir_root = os.path.join(config.get(prefs.SUPPORT_DIRECTORY),
                                 'Bittornado')
        ...
        for icon in GetIcons():
            ...
            if not os.path.exists(i):
                # There is no OLDICONPATH in Democracy because
                # sys.argv isn't guaranteed to be defined --NN
