<?php
     ...
     *	@version	$Id: browser.php,v 1.1 2005/08/20 13:36:02 mitchc2 Exp $
     */
    ...
    class SimpleBrowser {
         ...
         *    @access public
         */
        function post($url, $parameters = false, $files = false) {
            if (! is_object($url)) {
                $url = new SimpleUrl($url);
            }
            if ($this->getUrl()) {
	            ...
	            return $this->_load($url, new SimplePostEncoding($parameters));						
							...
							$enc = new SimpleMultipartEncoding($parameters);
							foreach( $files as $f ) {
								$enc->attach($f["key"], $f["content"], $f["filename"], $f["mimetype"]);
							}
	            return $this->_load($url, $enc );
						}
        }
