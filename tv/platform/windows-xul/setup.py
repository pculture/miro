...
#        moduleGraph.graphreport()
    ...
    def fillTemplates(self):
        ...
        for lang in glob (os.path.join (xulBase, 'chrome', 'locale', '*')):
            ...
            self.fillTemplate(os.path.join(lang, 'startup.dtd'))
            self.fillTemplate(os.path.join(lang, 'canscrape.dtd'))
            self.fillTemplate(os.path.join(lang, 'migrate.dtd'))
...
class bdist_xul (bdist_xul_dumb):
    def run(self):
        ...
        nsisVars['CONFIG_DL_EXECUTABLE'] = "%s.exe" % (self.getTemplateVariable('shortAppName'))
