...
class Rerequester:
    ...
    def rerequest(self, s, callback):
        if not self.lock.isfinished():  # still waiting for prior cycle to complete??
            ...
            return
        self.lock.reset()
        # Added name to thread for Democracy --NN
        rq = Thread(target = self._rerequest, args = [s, callback], name = "BitTornado -- rerequest")
        rq.setDaemon(False)
        rq.start()
    ...
    def rerequest_single(self, t, s, callback):
        l = self.lock.set()
        # Added name to thread for Democracy --NN
        rq = Thread(target = self._rerequest_single, args = [t, s+get_key(t), l, callback], name = "BitTornado -- rerequest_single")
        rq.setDaemon(False)
        rq.start()
        self.lock.wait()
