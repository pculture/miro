...
from miro.descriptions import (Artist, Album, AlbumEntry, CoverArt, LibraryItem,
    Label, Production, Genre, Rating, File, Media, Download, MediaType,
    Duration)
# XXX Description subclasses can't be modified; it might be useful to have some
# sort of schema-level support for enforcing that, and it could potentially
...
class CoverArtSchema(DDBObjectSchema):
    ...
    fields = DDBObjectSchema.fields + [
        ('record_id', SchemaId(RecordSchema)),
        ('album_id', SchemaId(AlbumSchema, noneOk=True)),
        ('path', SchemaFilename()),
    ]
...
class MediaTypeSchema(DDBObjectSchema):
    klass = MediaType
...
class DurationSchema(DDBObjectSchema):
    klass = Duration
    table_name = 'duration'
    fields = DDBObjectSchema.fields + [
        ('record_id', SchemaId(RecordSchema)),
        ('milliseconds', SchemaInt()),
    ]
...
object_schemas = [
    ...
    DownloadSchema, DurationSchema, MediaTypeSchema,
]
