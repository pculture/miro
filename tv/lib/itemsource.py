...
class SharingItemSource(ItemSource):
    ...
    def _item_info_for(self, item):
        return messages.ItemInfo(
            ...
            mime_type = item.enclosure_type,
            artist = item.artist if item.artist else u'',
            album = item.album if item.album else u'',
            track = item.track if item.track else -1,
            year = item.year if item.year else -1,
            genre = item.genre if item.genre else u'',
            rating = item.rating,
            auto_rating = None,
